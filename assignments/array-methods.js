...
let fullName = [];
runners.forEach(function(cv) {
  fullName.push(cv.first_name + " " + cv.last_name);
});
console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = runners.map(function(cv) {
  return cv.first_name.toUpperCase();
});
console.log(allCaps);
...
let largeShirts = runners.filter(function(cv) {
  return cv.shirt_size === "L";
});
...
let ticketPriceTotal = runners.reduce((sum, total) => sum + total.donation, 0);
console.log(`Donation Total: $${ticketPriceTotal}`);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 - LAST NAME UPPERCASE
let allCapsLast = runners.map(function(cv) {
  return cv.last_name.toUpperCase();
});

console.log(allCapsLast);
// Problem 2 - create a list of the companies that are being sponsored.

// let company = runners.forEach(function(cv) {
//   return company.push(cv.company_name);
// });
// console.log(company);
let newCompyArr = [];
let company = runners.forEach(function(cv) {
  return newCompyArr.push(cv.company_name);
});
console.table(newCompyArr);
